...
export const NovelCommentSection = ({ novelId }: Props) => {
  const { user } = useAuth();
  ...
  const fetchComments = async (pageNum: number = 0) => {
    setLoading(true);
    try {
      const res = await getCommentsByNovel(novelId, pageNum, 5); // size=5
      setComments(res.data.content);
      setTotalPages(res.data.totalPages);
      setPage(pageNum);
    } catch (err) {
      console.error("Failed to fetch novel comments:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      console.error("Failed to add novel comment:", err);
    }
  };
  ...
  const handleDeleteComment = async (id: number) => {
    try {
      ...
      console.error("Failed to delete novel comment:", err);
    }
  };
      ...
      {user ? (
        <div className="flex gap-2">
          <input
            ...
            placeholder="Write a comment about this novel..."
